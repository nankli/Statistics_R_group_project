...
$$
\text{log(Box office)} = \beta_0 + \beta_1*\text{log(Budget)} + \beta_2*\text{duration} + \beta_3*\text{maturity_rating} + \beta_4*\text{genre_primary} + \beta_5*\text{meta_critic_rating}
$$
...
```
```{r no perfect collinearity check}
