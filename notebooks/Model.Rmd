...
model_a_table <- stargazer(model_1, model_2, model_3, model_4, model_5, type = "text",
          header=FALSE, no.space=TRUE, column.sep.width = "3pt",
          #se = list(se.model_1, se.model_2, se.model_3, se.model_4),
          star.cutoffs = c(0.05, 0.01, 0.001), title = "Summary for models 1 to 5")
```
```{r release_month model compare to summer & thanksxmax}
...
```
```{r r code and plots assessing normally distributed errors}
