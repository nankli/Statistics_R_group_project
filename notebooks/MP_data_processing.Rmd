...
post_2000_raw$genre_primary <- str_extract(post_2000_raw$genre,"(\\w+)") 
...
```
```{r histogram for proposal}
post_2000_raw_histogram <- post_2000_raw %>%
filter(box_office != "" & budget != "") %>%
  ggplot() + 
  aes(x = clean_budget, y = clean_box_office, color = genre_primary) +
  geom_point() +
