...
box_office_hist <- post_1990_filtered %>%
  ...
  labs(title = 'Distribution of box office', x = 'Box office (Million $)', y = 'Frequency')

log_box_office_hist <- post_1990_filtered %>%
  ggplot() + aes(x=log(clean_box_office_real)) + 
  geom_histogram(position ='dodge', bins=30) + 
  labs(title = 'Distribution of log(Box office)', x = 'log(Box office) (Million $)', y = 'Frequency')
...
budget_hist <- post_1990_filtered %>%
  ...
  geom_histogram(position ='dodge', bins=30) + 
  labs(title = 'Distribution of budget', x = 'Budget (Million $)', y = 'Frequency')

log_budget_hist <- post_1990_filtered %>%
  ggplot() + aes(x=log(clean_budget_real)) + 
  geom_histogram(position ='dodge', bins=30) + 
  labs(title = 'Distribution of log(Budget)', x = 'log(Budget) (Million $)', y = 'Frequency')
...
residual_hist <- hist(model_3$residuals)
```
...
```{r transform plots, fig.cap='Data Transformations', fig.pos='!b', fig.height = 2.5}
...
```{r correlation plots, fig.cap='Outlier box plots', fig.pos='!b', fig.height = 4, warning=FALSE, message=FALSE}
corr_matrix
```
...
```{r LCE, fig.cap='Outlier box plots', fig.pos='!b', fig.height = 3, warning=FALSE, message=FALSE}
resid_plot
```

# Model Results and Intrepretation

```{r core model outputs}
core_model_table
```
...
```{r box and bar plots, fig.cap='Outlier box plots', fig.pos='!b', fig.height = 3}
(box_plot_office | box_plot_budget)
```

```{r sensitivity model outputs}
